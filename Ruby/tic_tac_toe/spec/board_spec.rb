...
module TicTacToe
	describe Board do 
		...
		context "#set_cell" do
			it "updates the value of a cell object at a (x, y) coordinate" do
				Rabbit = Struct.new(:value)
				grid = [[Rabbit.new("Watson"), "", ""],["", "", ""], ["", "", ""]]
				board = Board.new(grid: grid)
				board.set_cell(0, 0, "hop")
				expect(board.get_cell(0, 0).value).to eq "hop"
			end
		end
